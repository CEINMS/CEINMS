...
void printTrial(const TrialData& rhs) {

	cout << "ID " << rhs.id_ << std::endl;
	cout << "noMuscles " << rhs.noMuscles_ << std::endl;
    cout << "noEMGsteps " << rhs.noEmgSteps_ << std::endl; 
    cout << "noLmtsteps " << rhs.noLmtSteps_ << std::endl; 
	cout << "noDofs " << rhs.noDoF_ << std::endl; 
	cout << "noTorqueSteps " << rhs.noTorqueSteps_ << std::endl; 
}
...
//    string newCalibratedSubjectXmlFile("calibratedSubject.xml");
	...
	("osim-model,m", po::value<string>(&osimModelFile), "valid OpenSim model")
	...
	calibrationXmlReader.getTrialsDirectory(trialsInputDirectory);
    ...
    std::cout << "NMSModelCfg " << runMode << std::endl;
     switch(runMode) {
        ...
        case NMSModelCfg::OpenLoopExponentialActivationStiffTendonOffline: {
            ...
            InputDataInterpreterOsim inputData(muscleNames, dofNames);
			inputData.setMuscleNamesOnDofs(muscleNamesOnDofs);
            inputData.setInputDirectory(trialsInputDirectory);
			inputData.setOsimModel(osimModelFile);
