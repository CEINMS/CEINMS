...
void runThreads(T1& t1, T2& t2, T3& t3) {
...
int main(int argc, char** argv) {
  ...
  cout << "Check configuration data...\n";
    ...
    ("osim-model,m", po::value<string>(&osimModelFile), "OpenSim model file")
    ("subject,s", po::value<string>(&subjectFile), "subject xml file")
	("input-dir,i",po::value<string>(&inputDir), "Input directory")
    ...
    ("output-dir,o",po::value<string>(&outputDir)->default_value("Output"), "Output directory");
    
    po::variables_map vm;
    po::store(po::parse_command_line(argc, argv, desc), vm);
    po::notify(vm);    

    if (vm.count("help") || argc < 9) {
        cout << desc << "\n";
        return 1;
    }
...
//    ExternalTorqueFromFile externalTorqueProducer(inputDirectory);
 ...
 //   NMSModelCfg::RunMode runMode = NMSModelCfg::OpenLoopExponentialActivationStiffTendonOnline;
    ...
    switch(runMode) {
        ...
        case NMSModelCfg::OpenLoopExponentialActivationStiffTendonOnline: {
            ...
            vector<string> muscles, dofNames;
            vector<vector<string> > musclesOnDofs;
            mySubject.getMuscleNames(muscles);
            mySubject.getDoFNames(dofNames);
            mySubject.getMuscleNamesOnDofs(musclesOnDofs);
            ProducersFromOpenSim producersFromOpenSim(osimModelFile,ikMotFilename, muscles, musclesOnDofs, dofNames);
			ModelEvaluationOnline<MyNMSmodel> consumer(mySubject, outputDir);
            runThreads(consumer, emgProducer, externalTorqueProducer);
			producersFromOpenSim.join();
            break;
        }
        ...
        case NMSModelCfg::OpenLoopExponentialActivationStiffTendonOffline: {
            ...
            mySubject.getMuscleNamesOnDofs(musclesOnDofs);
            ProducersFromOpenSim producersFromOpenSim(osimModelFile,ikMotFilename, muscles, musclesOnDofs, dofNames);
            ModelEvaluationOffline<MyNMSmodel> consumer(mySubject, outputDir);
            runThreads(consumer, emgProducer, externalTorqueProducer);
			producersFromOpenSim.join();
            break;
        }
        ...
        */
        ...
        case NMSModelCfg::HybridExponentialActivationStiffTendonOnline: { 
            ...
            ModelEvaluationHybrid<MyNMSmodel, MyErrorMinimizer> consumer(mySubject, errorMinimizer, outputDir);
            vector<string> muscles, dofNames;
            vector<vector<string> > musclesOnDofs;
            mySubject.getMuscleNames(muscles);
            mySubject.getDoFNames(dofNames);
