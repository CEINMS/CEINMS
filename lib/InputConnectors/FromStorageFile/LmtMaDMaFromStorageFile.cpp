...
#include "ceinms/LmtMaDMaFromStorageFile.h"
...
namespace ceinms {
    void LmtMaDMaFromStorageFile::operator()()
    {
        ...
        while (lmtData_.areStillData())
        {
            ...
            for (unsigned int currentDof = 0; currentDof < dofNames_.size(); ++currentDof)
            {
                ...
                (*dMaDataStorageFiles_.at(currentDof)).readNextData();
                auto newDMaData = (*dMaDataStorageFiles_.at(currentDof)).getCurrentData();

                vector<double> selectedDMaData(musclePositionsInDMaStorages_.at(currentDof).size());
                for (int i = 0; i < selectedDMaData.size(); ++i)
                    selectedDMaData.at(i) = newDMaData.at(musclePositionsInDMaStorages_.at(currentDof).at(i));

                updateMomentArmDerivatives(selectedDMaData, time, currentDof);
            }
        }
        ...
        for (unsigned int i = 0; i < dofNames_.size(); ++i) {
            ...
            updateMomentArmDerivatives(endOfData, std::numeric_limits<double>::infinity(), i);
        }
